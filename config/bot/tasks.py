...
async def send_async_message(chat_id, text, phone):
    ...
    try:
        ...
        sms_service.send_sms(message=text, number=phone)
        sms = Sms()
        sms.counter()
...
async def send_async_messages_to_admin(chat_id, text):
    """
    Sends a message to an admin chat_id asynchronously.
    """
    try:
        await bot.send_message(chat_id, text=text, parse_mode=ParseMode.HTML)
        logging.info(f"Message successfully sent to admin chat_id {chat_id}.")
    except Exception as e:
        logging.error(f"Failed to send message to admin chat_id {chat_id}: {e}")
...
def send_daily_message():
    ...
    for payment in upcoming_payments:
        ...
        try:
            async_to_sync(send_async_message)(user_chat_id, message_text, payment.user.phone)
        except Exception as e:
            logging.error(f"Failed to send message to chat_id {user_chat_id}: {e}")

    logging.info("Celery task completed: Daily messages sent.")
